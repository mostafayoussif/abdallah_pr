...
edit_trainee_wt.grid(row = 0, column = 2, padx = 20)
...
# Add the missing functionality for trainees
def get_all_trainees():
    ...
    for trainee in data.keys():
        ...
        name = Data_Label(data_frame, width = 400, text = trainee_data['Name'])
        name.grid(row = j, column = 0, padx = 30)
        
        et = Data_Label(data_frame, width = 150, text = trainee_data['Exercises type'])
        et.grid(row = j, column = 1, padx = 80)
       ...
       # status.grid(row = j, column = 6, padx = 20)
        ...
        delete_trainee_button.configure(command = lambda name = trainee, trainee_data_button = data_frame, 
                                        trainee_update_button = update_trainee_button, trainee_delete_button = delete_trainee_button: delete_trainee(name, trainee_data_button, trainee_update_button, trainee_delete_button) )
        delete_trainee_button.grid(row = j, column = 2, padx = 5)
        j += 1

def delete_trainee(trainee_name, tb, tu, td):
    deletion_confirmation = messagebox.askyesno('confirm_deletion', f'Do you want to delete {trainee_name} permanently?')
    if deletion_confirmation == 1:
        trainee_seletion = db.reference(f'members/trainees/{trainee_name}').delete()
        tb.grid_forget()
        tu.grid_forget()
        td.grid_forget()
    else:
